...
This service is one of four Beyond primitives (auth, kv, queue, objects). **When in doubt, look at a sibling.** Error shapes, middleware stacks, config patterns, allocator setup, xtask structure, REST and SDK conventions, mise tasks, CI workflows — all four services should feel like they came from the same hand. Before introducing a new pattern, check `../auth`, `../kv`, or `../queue` first. If you find a divergence between this service and a sibling, treat it as a bug worth fixing.
